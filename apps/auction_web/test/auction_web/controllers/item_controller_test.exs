defmodule AuctionWeb.ItemControllerTest do  
    ...
    test "GET /", %{conn: conn} do  
        {:ok, _item1} = Auction.Repo.insert(%Auction.Item{title: "Item1"})  
        conn = get conn, "/"    
        assert html_response(conn, 200) =~ "<h1>Items</h1>"  
        assert html_response(conn, 200) =~ "Item1"   
    end 
    ...
    describe "POST /items" do          
        ...
        test "with valid params, redirects to the new Item", %{conn: conn} do      
            conn = post conn, "/items", %{"item" => %{"title" => "Item1"}}      
