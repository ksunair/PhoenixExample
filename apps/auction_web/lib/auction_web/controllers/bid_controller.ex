defmodule AuctionWeb.BidController do
    ...
    def create(conn, %{"bid" => %{"amount" => amount}, "item_id" => item_id}) do
        ...
        case Auction.Bid.insert_bid(%{amount: amount, item_id: item_id, user_id: user_id}) do
            {:ok, bid} -> redirect conn, to: AuctionWeb.Router.Helpers.item_path(conn, :show, bid.item_id)
            {:error, bid} -> 
    ...
    defp require_logged_in_user(%{assigns: %{current_user: nil}} = conn, _opts) do
        ...
        |> redirect(to: Aunction.Route.Helpers.item_path(conn, :index))
        |> halt()
    end
